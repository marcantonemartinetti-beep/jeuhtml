...
class Monster {
  ...
  die() {
    ...
    // Card Drop Logic: physical drops in world (duplicates allowed).
    if (typeof cardDrops !== 'undefined' && typeof MobCardDrop === 'function') {
      try {
        if (!GameState.saveData.cards) GameState.saveData.cards = [];
        const dropChance = 1.0;
        if (Math.random() < dropChance) {
          cardDrops.push(new MobCardDrop(this.root.position.clone(), this.T));
        }
      } catch (dropErr) {
        console.warn('Card drop spawn failed:', dropErr);
      }
    }
    ...
    if (this.boss) {
      ...
      if (typeof chests !== 'undefined' && typeof BossChest === 'function') {
        const rewards = 1 + Math.floor(Math.random() * 3);
        chests.push(new BossChest(this.root.position.clone(), rewards));
      }
    }
  }
}
